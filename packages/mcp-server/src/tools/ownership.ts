...
export class OwnershipTools {
  ...
  private async assignActorOwnership(args: any) {
    ...
    for (const actor of actors) {
      for (const player of players) {
        try {
          const result = await this.foundryClient.query('jdr-mcp-bridge.setActorOwnership', {
            actorId: actor.id,
            userId: player.id,
  ...
  private async listActorOwnership(args: any) {
    ...
    try {
      const ownershipData = await this.foundryClient.query('jdr-mcp-bridge.getActorOwnership', {
        actorIdentifier,
        playerIdentifier,
      });
  ...
  private async resolveActors(identifier: string): Promise<Array<{ id: string; name: string }>> {
    ...
    try {
      if (identifier.toLowerCase().includes('all friendly npcs')) {
        ...
        const actors = await this.foundryClient.query('jdr-mcp-bridge.getFriendlyNPCs', {});
        ...
        const actors = await this.foundryClient.query('jdr-mcp-bridge.getPartyCharacters', {});
        this.logger.debug(`Found ${actors.length} party characters`);
        return actors;
      } else {
        // Single actor lookup
        this.logger.debug(`Looking for single actor: ${identifier}`);
        const actor = await this.foundryClient.query('jdr-mcp-bridge.findActor', {
          identifier,
        });
    ...
    try {
      if (identifier.toLowerCase() === 'party') {
        ...
        const players = await this.foundryClient.query(
          'jdr-mcp-bridge.getConnectedPlayers',
          {}
        );
        ...
        const players = await this.foundryClient.query('jdr-mcp-bridge.findPlayers', {
          identifier,
